...
func (d *Device) Configure(cfg Config) {
	...
	d.SendCommand(0x20) //g vcom
	for count := 0; count < 15; count++ {
		d.SendData(lut_vcom0[count])
	}
	d.SendCommand(0x21) //g ww --
	for count := 0; count < 15; count++ {
		d.SendData(lut_w[count])
	}
	d.SendCommand(0x22) //g bw r
	for count := 0; count < 15; count++ {
		d.SendData(lut_b[count])
	}
	d.SendCommand(0x23) //g wb w
	for count := 0; count < 15; count++ {
		d.SendData(lut_g1[count])
	}
	d.SendCommand(0x24) //g bb b
	for count := 0; count < 15; count++ {
		d.SendData(lut_g2[count])
	}
	d.SendCommand(0x25)
	for count := 0; count < 15; count++ {
		d.SendData(lut_vcom1[count])
	}
	d.SendCommand(0x26)
	for count := 0; count < 15; count++ {
		d.SendData(lut_red0[count])
	}
	d.SendCommand(0x27)
	for count := 0; count < 15; count++ {
		d.SendData(lut_red1[count])
	}
}
...
func (d *Device) Display() error {
	...
	for i := uint32(0); i < d.bufferLength; i++ {
		...
		for bit := uint8(0); bit < 4; bit++ {
			if (d.buffer[BLACK-1][i] & (0x80 >> bit)) != 0 {
				temp |= 0xC0 >> (bit * 2)
			}
		}
		d.SendData(temp)
